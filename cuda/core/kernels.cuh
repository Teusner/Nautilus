...
__global__ void Rxx(float dt, float* Rx, float* Uxx, float* Uyy, float* Uzz, unsigned int* S, float* eta_tau_gamma_p, float* mu_tau_gamma_s, float* tau_sigma) {
    ...
    if (i>=2 && i<x-2 && j>=2 && j<y-2 && k>=2 && k<z-2) {
        unsigned int material_index = S[k + z*(j + y*i)];
        Rx[k + z*(j + y*i)] = (1 - dt / tau_sigma[0]) * Rx[k + z*(j + y*i)] - dt / tau_sigma[0] * (eta_tau_gamma_p[material_index] * (Uxx[k + z*(j + y*i)] + Uyy[k + z*(j + y*i)] + Uzz[k + z*(j + y*i)]) - 2 * mu_tau_gamma_s[material_index] * (Uyy[k + z*(j + y*i)] + Uzz[k + z*(j + y*i)]));
    }
}
...
__global__ void Ryy(float dt, float* Ry, float* Uxx, float* Uyy, float* Uzz, unsigned int* S, float* eta_tau_gamma_p, float* mu_tau_gamma_s, float* tau_sigma) {
    ...
    if (i>=2 && i<x-2 && j>=2 && j<y-2 && k>=2 && k<z-2) {
        unsigned int material_index = S[k + z*(j + y*i)];
        Ry[k + z*(j + y*i)] = (1 - dt / tau_sigma[0]) * Ry[k + z*(j + y*i)] - dt / tau_sigma[0] * (eta_tau_gamma_p[material_index] * (Uxx[k + z*(j + y*i)] + Uyy[k + z*(j + y*i)] + Uzz[k + z*(j + y*i)]) - 2 * mu_tau_gamma_s[material_index] * (Uxx[k + z*(j + y*i)] + Uzz[k + z*(j + y*i)]));
    }
}
...
__global__ void Rzz(float dt, float* Rz, float* Uxx, float* Uyy, float* Uzz, unsigned int* S, float* eta_tau_gamma_p, float* mu_tau_gamma_s, float* tau_sigma) {
    ...
    if (i>=2 && i<x-2 && j>=2 && j<y-2 && k>=2 && k<z-2) {
        unsigned int material_index = S[k + z*(j + y*i)];
        Rz[k + z*(j + y*i)] = (1 - dt / tau_sigma[0]) * Rz[k + z*(j + y*i)] - dt / tau_sigma[0] * (eta_tau_gamma_p[material_index] * (Uxx[k + z*(j + y*i)] + Uyy[k + z*(j + y*i)] + Uzz[k + z*(j + y*i)]) - 2 * mu_tau_gamma_s[material_index] * (Uxx[k + z*(j + y*i)] + Uyy[k + z*(j + y*i)]));
    }
}
...
__global__ void Rxy(float dt, float* Rxy, float* Ux, float* Uy, unsigned int* S, float* mu_tau_gamma_s, float* tau_sigma) {
    ...
    if (i>=2 && i<x-2 && j>=2 && j<y-2 && k>=2 && k<z-2) {
        ...
        unsigned int material_index = S[k + z*(j + y*i)];
        Rxy[k + z*(j + y*i)] = (1 - dt / tau_sigma[0]) * Rxy[k + z*(j + y*i)] - dt / tau_sigma[0] * mu_tau_gamma_s[material_index] * (Uxy + Uyx);
    }
}
...
__global__ void Ryz(float dt, float* Ryz, float* Uy, float* Uz, unsigned int* S, float* mu_tau_gamma_s, float* tau_sigma) {
    ...
    if (i>=2 && i<x-2 && j>=2 && j<y-2 && k>=2 && k<z-2) {
        ...
        unsigned int material_index = S[k + z*(j + y*i)];
        Ryz[k + z*(j + y*i)] = (1 - dt / tau_sigma[0]) * Ryz[k + z*(j + y*i)] - dt / tau_sigma[0] * mu_tau_gamma_s[material_index] * (Uyz + Uzy);
    }
}
...
__global__ void Rxz(float dt, float* Rxz, float* Ux, float* Uz, unsigned int* S, float* mu_tau_gamma_s, float* tau_sigma) {
    ...
    if (i>=2 && i<x-2 && j>=2 && j<y-2 && k>=2 && k<z-2) {
        ...
        unsigned int material_index = S[k + z*(j + y*i)];
        Rxz[k + z*(j + y*i)] = (1 - dt / tau_sigma[0]) * Rxz[k + z*(j + y*i)] - dt / tau_sigma[0] * mu_tau_gamma_s[material_index] * (Uxz + Uzx);
    }
}
